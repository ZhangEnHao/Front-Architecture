...
export const onFinallyPublic = (allPublicResUlt) => {
  setTimeout(() => {
    if (
      Object.prototype.toString.call(allPublicResUlt.onFinally) ===
      "[object Array]"
    ) {
 ...
 */
export const allPublic = function (
  value: Array<any>,
  type: string = "all"
  ...
  const allPublicResUlt = new PromiseClass((resolve1, reject1) => {
    const InquireFun = () => {
      new PromiseClass((resolve2, reject2) => {
        if (
          resUlt_resolve.filter((e) => e !== "[object Empty]").length === lng
        ) {
          // 全部成功
 ...
 * @param bool 是否失败
 */
export const resultResolve = function (
  onfulfilled: Array<(value: any) => any>,
  onrejected: Array<(value: any) => any>,
  arg: Array<any>,
  bool: boolean = true
) {
  if (onfulfilled) {
    if (typeof onfulfilled[0] === "function") {
      ...
      if (
        value &&
        value.constructor &&
        value.constructor.name === "PromiseClass"
      ) {
        value
          .then((res) => {
            if (bool) {
